...
#######################################
## Local getter function for objects ##
## (better readability)              ##
#######################################
def gt(oname,whichone=0,val=True):
    ''' If you call it like this...
        
        gt('plant)
        
        ...you will get all the information the game knows about that object
        if it exists, and an error message if it does not.
        If you call it like this...
        
        gt('plant',val=False)
        
        ...you will get its position in the objects dictionary
        If there are more than one objects that share the same main picture
        you can specify which of them to get with the whichone optional argument
    '''
    return escape_addons.get_prop(oname,whichone=whichone,val=val,env=globals())
...
def generate_map():
    ...
    # if the room is outside
    if current_room in range(1, 21):
        bottom_edge = 2 # soil
        side_edge = 2 #soil
    # if the room is on the edge between outside and inside
    if current_room in range(21, 26):
        bottom_edge = 1 #wall
        side_edge = 2 # soil
    # if the room is inside
    if current_room >25:
        bottom_edge = 1 # wall
        side_edge = 1 # wall
    ...
    if current_room % MAP_WIDTH != 1:
        ...
        # if room on the left has a right exit, add left exit
        if room_to_left[4]:
    ...
    if current_room <= MAP_SIZE - MAP_WIDTH:
        ...
        # if room below has top exit, add bottom exit
        if room_below[3]:
